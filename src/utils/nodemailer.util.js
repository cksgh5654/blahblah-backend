...
const sendMailForResetPassword = async (email, otp) => {
  const mailOptions = {
    from: config.nodemailer.authEmail,
    to: email,
    subject: "Blahblah 비밀번호 재설정 otp 요청입니다.",
    html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #f9f9f9;">
              <h2 style="color: #333;">${email} 비밀번호 재설정 OTP 요청</h2>
              <p style="color: #555; font-size: 16px;">OTP를 입력하여 비밀번호 설정을 완료하세요.</p>
              <div style="margin: 20px 0; padding: 10px; font-size: 24px; font-weight: bold; color: #fff; background-color: #007BFF; border-radius: 5px; display: inline-block;">
              ${otp} 
              </div>
              <p style="color: #888; font-size: 14px;">5분간 유효합니다.</p>
